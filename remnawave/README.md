...
正式用户入口为 `https://sub-verizon.bigpandas.top/<short-uuid>`，现在直接返回
Mihomo YAML，不再显示官方 Subscription Page。旧 `/api/sub/<short-uuid>` 继续
兼容。版本控制中的 `mihomo-template.yaml` 是生产默认 Mihomo 模板，使用
`apply_subscription_template.py` 应用或检查。
响应规则中的通用 Clash 客户端（包括 Clash Mi、ClashMetaForAndroid 等）统一返回
该 Mihomo 模板；使用 `apply_subscription_response_rules.py` 应用或检查。
