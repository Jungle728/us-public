...
- 提供 Reality、Hysteria2、TUIC、AnyTLS 桌面入站，以及一个直连 AaITR 的认证 SOCKS5 转发入口。
...
| `32444/udp` | TUIC，实际端口以恢复后的 s-ui 入站配置为准 |
| `32445/tcp` | AnyTLS，实际端口以恢复后的 s-ui 入站配置为准 |
...
- `bigpandas.top` 的所有子域名显式走 `DIRECT`；`shu26.cfd` 显式走 `EXIT-MODE`，DNS 同样随代理解析。
...
- `bigpandas.top` 和 H100 SSH 域名始终直连并返回真实 IP；`shu26.cfd` 走代理。
