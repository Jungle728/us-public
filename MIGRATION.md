...
当前 AaITR 机器是管理源和迁移汇总点。GitHub 仓库只保存公开模板、脚本、前端构建产物和部署说明；生产数据库、面板密码、订阅凭据、证书私钥和生成的 CStoneCloud 配置只保存在 AaITR 的受限迁移目录中。
...
5. 先运行配置检查，再启动服务；最后运行 `python3 ./verify_s_ui.py all` 和 CStoneCloud 的 Compose 校验。
